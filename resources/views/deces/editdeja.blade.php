    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white shadow-xl rounded-lg overflow-hidden">
            ...
            <div class=" px-6 py-5" style="background-color: #6777ef">
                ...
                <p class="text-white mt-1">Gestion des extraits de décès</p>
            </div>
            
            <!-- Form Container -->
            <div class="p-8">
                <form action="{{ route('decesdeja.updateEtat', $decesdeja->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('POST')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <input type="text" id="nomDefunt" 
                                   class="block w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                                   value="{{ $decesdeja->user ? $decesdeja->user->name .' '.$decesdeja->user->prenom : 'Demandeur inconnu' }}" 
                                   disabled>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="etat" id="etat" 
                                class="block w-full px-4 py-3 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150">
                            @foreach($etats as $etat)
                                <option value="{{ $etat }}" {{ $decesdeja->etat == $etat ? 'selected' : '' }} class="py-2">
                                    {{ ucfirst($etat) }}
                                </option>
                            @endforeach
                        </select>
