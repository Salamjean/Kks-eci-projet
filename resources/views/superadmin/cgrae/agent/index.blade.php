      ...
      <div class="col-lg-12">
        <div class="card mb-4">
          <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold " style="color:green">Informations des agents</h6>
          </div>
            ...
            <table class="table align-items-center table-flush" id="dataTable">
                ...
                <tbody>
                    ...
                    <tr style="font-size: 12px">
                        ...
                        <td>{{ $agent->commune }}</td>
                        <td>
                            <form action="{{ route('cgraeagent.edit', $agent->id) }}" method="GET">
                                @csrf
                                <button style="margin-left:50%" type="submit" ><a href="{{ route('cgraeagent.edit', $agent->id) }}" class="edit"><i class="fas fa-edit"></i></a></button>
                            </form>
                        </td>
                        <td>
                        <form id="deleteForm{{ $agent->id }}" action="{{ route('cgraeagent.archive', $agent->id) }}" method="POST" style="display: flex; justify-content:center; align-items:center">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="delete" onclick="confirmDelete({{ $agent->id }})"><i class="fas fa-trash"></i></button>
                        </form>
                    </td>
                    </tr>
                    @empty
