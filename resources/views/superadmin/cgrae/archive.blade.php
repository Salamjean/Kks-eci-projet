      ...
      <div class="col-lg-12">
        <div class="card mb-4">
          <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Liste de toutes les cgrae archivées</h6>
          </div>
                  ...
                  @forelse ($cgraes as $cgrae)
                      <tr style="font-size: 12px">
                          ...
                          <td class="text-center">
                              <button type="button" class="delete" onclick="confirmUnarchive('{{ $cgrae->id }}')">
                                <i class="fas fa-folder-open"></i>
                              </button>
                              <form id="unarchive-form-{{ $cgrae->id }}" action="{{ route('cgraes.unarchive', $cgrae->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('PUT')
                              </form>
                            </td>
                            <td class="text-center">
                              <button type="button" class="delete" onclick="confirmDelete('{{ $cgrae->id }}')">
                                <i class="fas fa-trash"></i>
                              </button>
                              <form id="delete-form-{{ $cgrae->id }}" action="{{ route('cgraes.delete', $cgrae->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                              </form>
                            </td>
