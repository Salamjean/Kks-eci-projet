...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">Listes des agences</h1>
      <ol class="breadcrumb">
      ...
      <div class="col-lg-12">
        <div class="card mb-4">
          <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold " style="color:green">Informations sur les agences</h6>
          </div>
          <div class="table-responsive p-3">
            ...
            <table class="table align-items-center table-flush" id="dataTable">
                <thead class="bg-navbar text-white" style="background-color: green">
                ...
                <tbody>
                    ...
                    <tr style="font-size: 12px">
                        ...
                        <td>
                            <form action="{{ route('cgraeagences.edit', $agence->id) }}" method="GET">
                                @csrf
                                <button style="margin-left:50%" type="submit" ><a href="{{ route('cgraeagences.edit', $agence->id) }}" class="edit"><i class="fas fa-edit"></i></a></button>
                            </form>
                        </td>
                        <td>
                        <form id="deleteForm{{ $agence->id }}" action="{{ route('cgraeagences.archive', $agence->id) }}" method="POST" style="display: flex; justify-content:center; align-items:center">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="delete" onclick="confirmDelete({{ $agence->id }})"><i class="fas fa-trash"></i></button>
                        </form>
                    </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="9" class="text-center">Aucune agence inscrite</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            function confirmDelete(agentId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        document.getElementById('deleteForm' + agentId).submit();
                    }
                });
            }
        </script>
