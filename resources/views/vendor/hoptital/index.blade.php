...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="page-title"><i class="fas fa-home me-2"></i>Liste des hôpitaux de votre mairie</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="{{ route('vendor.dashboard') }}"><i class="fas fa-home"></i> Accueil</a></li>
      <li class="breadcrumb-item active">Hôpital</li>
    </ol>
  </div>
    ...
    <div class="col-lg-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h6 class="m-0 font-weight-bold"><i class="fas fa-file-contract me-2"></i>Hôpital de votre mairie  </h6>
          ...
          <table class="table align-items-center" id="dataTable">
            <thead>
              <tr class="text-center">
                    ...
                    <th class="text-center">Nom et prénoms de l'administrateur</th>
                    <th class="text-center">Email de l'hôpital</th>
                    <th class="text-center">Contact de l'hôpital</th>
            ...
            <tbody>
                @forelse ($doctors as $doctor)
                        <tr style="font-size: 12px;  text-align-center">
                            <td class="text-center">{{ $doctor->nomHop }}</td>
                            <td class="text-center">{{ $doctor->name }}</td>
                            <td class="text-center">{{ $doctor->email }}</td>
                            <td class="text-center">{{ $doctor->contact }}</td>
                            <td class="text-center">{{ $doctor->commune }}</td>
                            <td class="text-center">
                                <form action="{{ route('doctor.hoptitaledit', $doctor->id) }}" method="GET">
                                    @csrf
                                    <button type="submit" ><a href="{{ route('doctor.hoptitaledit', $doctor->id) }}" class="edit"><i class="fas fa-edit"></i></a></button>
                                </form>
                            </td>
                            <td>
                                <form action="{{ route('doctor.hoptitaldelete', $doctor) }}" method="POST" id="deleteForm_{{ $doctor->id }}" style="display: flex; justify-content:center; align-items:center">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="delete" onclick="confirmDelete(event, '{{ $doctor->id }}')"><a href="#" class="delete"><i class="fas fa-trash"></i></button>
                                </form>
                            </td>
                    </tr>
                        @empty
